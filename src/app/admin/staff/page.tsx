...
export default async function AdminStaffPage() {
  ...
  const { data: staff } = await supabase.from("admins").select("*").order("role");

  return (
    <div className="space-y-12">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-10 border-b border-brand-navy/5">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-secondary italic mb-2">Team Management</p>
          <h1 className="text-4xl font-serif italic text-brand-navy tracking-tight uppercase">Admin <span className="text-secondary">Staff</span></h1>
        </div>
      </header>
        ...
        <div className="xl:col-span-4 max-w-sm w-full space-y-8 sticky top-10 self-start">
          <section className="bg-brand-navy p-8 lg:p-10 text-white shadow-xl">
            <div className="flex items-center gap-4 border-b border-white/10 pb-6 mb-8">
              ...
              <h3 className="text-[10px] font-bold uppercase tracking-[0.3em] text-secondary">Add Staff Member</h3>
            </div>

            <form action={async (formData) => { "use server"; await createStaffMember(formData); }} className="space-y-8">
              <div className="space-y-2">
                <label className="text-[9px] font-bold uppercase tracking-widest text-white/50 italic">Full Name</label>
              ...
              <div className="space-y-2">
                <label className="text-[9px] font-bold uppercase tracking-widest text-white/50 italic">Email Address</label>
              ...
              <div className="space-y-2">
                <label className="text-[9px] font-bold uppercase tracking-widest text-white/50 italic">Role</label>
        ...
        <div className="xl:col-span-8 flex flex-col gap-6">
          ...
          {(!staff || staff.length === 0) && (
            <div className="py-32 text-center bg-white border border-brand-navy/5 shadow-sm">
              ...
              <h3 className="text-xl font-serif italic text-brand-navy uppercase tracking-widest">No Staff Members Yet</h3>
              <p className="text-xs text-brand-navy/30 uppercase tracking-widest mt-2">Add your first team member using the form.</p>
            </div>
          )}
        </div>
      </div>
